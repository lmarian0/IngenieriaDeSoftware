...
public class EnemySpawner implements Runnable {
    ...
    /*
        ...
        Ej: TrapitoFactory, el cual tenga un metodo como el siguiente:
            generatedEnemies.add(gobSpawner.createEnemy(10,20));
     ...
     */
    private GoblinFactory gobSpawner;
    private boolean running;
    private List<Enemy> generatedEnemies;

    public EnemySpawner() {
        this.gobSpawner = new GoblinFactory();
        running = true;
    ...
    public void run() {
        ...
        while(running) {
            int delay = 3000 + rand.nextInt(5000);
            try {
                Thread.sleep(delay);
